...
from lib.variables import Catches, Fish_Catches, GoodPoke, EggsNum
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
     ...
     if any(word in str(message.channel.id) for word in channelids):
            ...
            if message.author.id != 664508672713424926: #Whitelist Pokemeow
                if message.author.id != 833428629705719828: #Whitelist Shadow Lugia
                    if message.author.id != 795050413279019028: #Whitelist Meow Helper
                        if message.author.id != 754693460413644800:
                          quit()
         ...
         if "egg is ready" in message.content:
             await eggs(message, client)
